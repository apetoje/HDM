...
# ======================================================================
# All 18 octane isomers with central entropy (Scott's correlation)
# ======================================================================
isomers = [
    ("n-Oktan",                 111.67),
    ("2-Metilheptan",           109.84),
    ("3-Metilheptan",           111.26),
    ("4-Metilheptan",           109.32),
    ("2,2-Dimetilheksan",       103.42),
    ("2,3-Dimetilheksan",       108.02),
    ("2,4-Dimetilheksan",       106.98),
    ("2,5-Dimetilheksan",       105.72),
    ("3,3-Dimetilheksan",       104.74),
    ("3,4-Dimetilheksan",       106.51),
    ("3-Etilheksan",            109.43),
    ("2,2,3-Trimetilpentan",    101.09),
    ("2,2,4-Trimetilpentan",    101.31),
    ("2,3,3-Trimetilpentan",    102.06),
    ("2,3,4-Trimetilpentan",    102.39),
    ("2,2,3,3-Tetrametilbutan",  93.06),
    ("3-Etil-2-metilpentan",    106.06),
    ("3-Etil-3-metilpentan",    101.48),
]

delta_S = 0.10          # experimental error
eps_target = 9.99e-17
Q_max = 120

print("=" * 80)
print("MHD SINGLE-PASS – COMPLETE INTERVAL ANALYSIS FOR ALL 18 OCTANE ISOMERS")
print("Experimental error: ±0.10 cal mol⁻¹ K⁻¹")
print("=" * 80)

for name, S0 in isomers:
    for delta in (-delta_S, 0.0, delta_S):
        S_val = S0 + delta
        C_target = mpf(1) / mpf(S_val)

        if delta < 0:
            label = f"{name} (donja)"
        elif delta == 0:
            label = f"{name} (centralna)"
        else:
            label = f"{name} (gornja)"

        print("=" * 70)
        print(f"MHD SINGLE-PASS DECOMPOSITION – {label}")
        print("=" * 70)
        print(f"S({name}) = {S_val:.2f} cal mol⁻¹ K⁻¹")
